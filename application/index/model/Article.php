...
class Article extends \think\Model
{
    ...
    {
		return \think\Db::table('exhibit')
			->alias('e')//设置别名
            ->field(['a.id', 'a.title', 'a.abstract', 'e.picture'])//指定字段查询
			/*
			'完整表名 别名’，返回行，查询，
			连接表article，连接条件为a.id = e.article_id
			
            ->join('article a', 'a.id = e.article_id')
            ->order('e.create_time DESC')//排序
            ->limit(5)->select();//获取数据集
    }
	*/
	public function getExhibits()
    {
		return \think\Db::table('article')
			->alias('a')//设置别名
            ->field(['id', 'title', 'abstract', 'image'])//指定字段查询
